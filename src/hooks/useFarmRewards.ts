...
export default function useFarmRewards() {
  ...
  const [sushiPrice, nativePrice, dpsPrice] = [
    useSushiPrice(),
    useNativePrice(),
    useDpsPrice()
  ]
  ...
  const map = (pool) => {
    ...
    function getRewards() {
      ...
      const defaultReward = {
        token: 'SUSHI',
        icon: 'https://raw.githubusercontent.com/deepspace-game/icons/main/token/sushi.jpg',
        rewardPerBlock,
      ...
      if (pool.chef === Chef.MASTERCHEF_V2) {
        ...
        const icon = ['0', '3', '4', '8'].includes(pool.id)
          ? `https://raw.githubusercontent.com/deepspace-game/icons/main/token/${pool.rewardToken.symbol.toLowerCase()}.jpg`
