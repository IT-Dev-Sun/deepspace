import { Currency, NATIVE, Token, WNATIVE } from '@deepspace-game/sdk'
...
import { useBytes32TokenContract, useTokenContract, useTokenPriceContract } from './useContract'
...
export function useDPSToken(): Token | undefined | null {
  return useToken(config.DPS_TOKEN_ADDRESS)
}

export function useDPSCurrency(): Currency | null | undefined {
  return useDPSToken()
}

export function useDpsPrice(): number {
  const [price, setPrice] = useState<number>()
  const contract = useTokenPriceContract()
  const fetchPrice = useCallback(async () => {
    async function getPrice() {
      if (!contract) {
        return 0
      }
      const price9 = await contract.getDpsTokenPrice()
      return price9 / 1e9
    }
