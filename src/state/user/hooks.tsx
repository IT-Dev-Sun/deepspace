...
import { FACTORY_ADDRESS, JSBI, Pair, Percent, Token, computePairAddress } from '@deepspace-game/sdk'
import {
  SerializedPair,
  SerializedToken,
  addSerializedPair,
  ...
  updateUserReferringCode,
  updateUserReferralAddress
} from './actions'
...
export function useSetUserReferralCode(): (refCode: string | '') => void {
  const dispatch = useAppDispatch()
  return useCallback(
    (userRefCode: string | '') => {
      dispatch(
...
export function useSetUserReferringCode(): (refCode: string | '') => void {
  const dispatch = useAppDispatch()
  return useCallback(
    (userRefingCode: string | '') => {
      dispatch(
        updateUserReferringCode({
          userReferringCode: userRefingCode,
        })
      )
    },
    [dispatch]
...
export function useSetUserReferringAddress(): (refAddr: string | '') => void {
  const dispatch = useAppDispatch()
  return useCallback(
    (userRefAddr: string | '') => {
      if (userRefAddr != ZERO_ADDRESS && userRefAddr != '') {
        const expires = new Date(new Date().getTime() + 24 * 60 * 60 * 1000) //  24 hours
        cookie.set("referrerAddress", userRefAddr, { expires: expires.toUTCString() });
      }
      if (userRefAddr == '') {
        userRefAddr = ZERO_ADDRESS
      }
      dispatch(
        updateUserReferralAddress({
          referringAddress: userRefAddr,
        })
      )
    },
    [dispatch]
...
export function useUserReferralCode(): string | '' {
  const userReferralCode = useAppSelector((state) => {
    return state.user.userReferralCode
  })
...
export function useUserReferringCode(): string | '' {
  const userReferringCode = useAppSelector((state) => {
    return state.user.userReferringCode
  })

  return useMemo(
    () => (userReferringCode === '' ? '' : userReferringCode),
    [userReferringCode]
  )
}

export function useUserReferringAddress(): string | '' {
  const userRefAddr = useAppSelector((state) => {
    return state.user.referringAddress
  })

  return useMemo(
    () => (
      userRefAddr === '' ? cookie.get("referrerAddress") : userRefAddr
    ),
    [userRefAddr]
  )
}
