...
import { ChainId, Currency, CurrencyAmount, DPS, Percent, Trade as V2Trade, TradeType } from '@deepspace-game/sdk'
import {
  EstimatedSwapCall,
  SuccessfulCall,
...
export function queryParametersToSwapState(parsedQs: ParsedQs, chainId: ChainId = config.DEFAULT_CHAIN): SwapState {
  let inputCurrency = parseCurrencyFromURLParameter(parsedQs.inputCurrency)
  let outputCurrency = parseCurrencyFromURLParameter(parsedQs.outputCurrency)
  const eth = 'ETH'
  const dps = DPS[chainId].address
  if (inputCurrency === '' && outputCurrency === '') {
    inputCurrency = eth
    outputCurrency = dps
  } else if (inputCurrency === '') {
    inputCurrency = outputCurrency === eth ? dps : eth
  } else if (outputCurrency === '' || inputCurrency === outputCurrency) {
    outputCurrency = inputCurrency === eth ? dps : eth
  }
